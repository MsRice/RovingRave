...
class Song:
    ...
    def add_new_song(cls, data):
        query = "INSERT INTO songs (title , artist , length , user_id , cover_link, track_link, spot_id) VALUE (%(title)s, %(artist)s , %(length)s, %(user_id)s , %(cover_link)s, %(track_link)s , %(spot_id)s);"
        ...
        song_id = connectToMySQL(
            'roving_rave_spotify_schema').query_db(query1, data)

        data['song_id'] = song_id[0]['id']

        query2 = "INSERT INTO crowd_queue (song_id , user_id) VALUE ( %(song_id)s , %(user_id)s) "
        connectToMySQL('roving_rave_spotify_schema').query_db(query2, data)
    ...
    def get_all_crowd_songs(cls):
        ...
        results = connectToMySQL('roving_rave_spotify_schema').query_db(query)
    ...
    def get_current(cls):
        ...
        if len(results) < 1:
            results = [{
                ...
                'artist': "N/A",
                'length': "00:00",
                'cover_link': "../static/img/no-img-avail.png"
            }]
        print(results)
        return results
